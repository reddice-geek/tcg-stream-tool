...
export default function App(){
  ...
  async function scanCameraCard(){
    ...
    detectingRef.current=true; setDetecting(true);
    try{
      ...
      const lines=String(result?.data?.text || '').split(/\r?\n/).map(cleanOcrText).filter(x=>x.length>=3);
      const attempts=[];
      for(const line of lines){ if(!attempts.includes(line)) attempts.push(line); }
      if(!attempts.includes(raw)) attempts.push(raw);
      const words=raw.split(' ').filter(w=>w.length>2);
      if(words.length>=2){
        const compact=words.slice(0,7).join(' ');
        if(!attempts.includes(compact)) attempts.push(compact);
      }

      let found=null;
      for(const candidate of attempts.slice(0,5)){
        found=await searchCard(null,candidate);
        if(found) break;
      }
      if(found){
        setQuery(found.name);
        setLastDetected(found.name);
        if(lastDetectedRef.current!==found.name){
          lastDetectedRef.current=found.name;
